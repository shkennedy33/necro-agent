...
# ============================================================================
# PATH setup — symlink nchat into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nchat command..."

NCHAT_BIN="$SCRIPT_DIR/venv/bin/nchat"
mkdir -p "$HOME/.local/bin"
ln -sf "$NCHAT_BIN" "$HOME/.local/bin/nchat"
echo -e "${GREEN}✓${NC} Symlinked nchat → ~/.local/bin/nchat"
...
if [ -n "$SHELL_CONFIG" ]; then
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
            echo "" >> "$SHELL_CONFIG"
            echo "# Necronomichat — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.nchat/skills/
# ============================================================================

HERMES_SKILLS_DIR="${NCHAT_HOME:-$HOME/.nchat}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nchat/skills/ ..."
...
echo ""
echo "Other commands:"
echo "  nchat status        # Check configuration"
echo "  nchat gateway install # Install gateway service (messaging + cron)"
echo "  nchat cron list     # View scheduled jobs"
echo "  nchat doctor        # Diagnose issues"
echo ""
