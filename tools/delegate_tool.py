...
def delegate_task(
    ...
    # Crystal routing: when a crystal tier is specified, use the crystal
    # resolver instead of the old delegation credentials path.
    # This lets the parent (Opus) say "delegate to a worker" and the child
    # automatically gets Sonnet-class credentials.
    if crystal:
        try:
            ...
            tier = tier_map.get(crystal.lower(), CrystalTier.WORKER)
            _crystal = resolve_crystal(tier, parent_agent=parent_agent)
            creds = {
                "model": _crystal.model or None,
                "provider": _crystal.provider or None,
                "base_url": _crystal.base_url or None,
                "api_key": _crystal.api_key or None,
                "api_mode": _crystal.api_mode,
            }
        except Exception as exc:
            logger.debug("Crystal resolution failed, falling back: %s", exc)
            try:
                creds = _resolve_delegation_credentials(cfg, parent_agent)
            except ValueError as exc2:
                return json.dumps({"error": str(exc2)})
    else:
        # Default: use WORKER crystal if available, otherwise fall back to
        # the old delegation credentials path.
        try:
            from nchat.crystals import resolve_crystal, CrystalTier
            _crystal = resolve_crystal(CrystalTier.WORKER, parent_agent=parent_agent)
            if _crystal.client is not None and _crystal.model:
                creds = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "crystal": {
                ...
                "description": (
                    "Which crystal tier to use for the subagent. "
                    "Default: worker (Sonnet-class). Use 'primary' only "
                    "for tasks requiring top-tier reasoning."
                ),
            },
        },
        "required": [],
...
registry.register(
    ...
    emoji="🔀",
    description_compact="Delegate a task to a sub-entity with its own context and tools.",
)
