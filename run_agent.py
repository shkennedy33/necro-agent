...
class AIAgent:
    ...
    def __init__(
        ...
        # Compact mode: use shorter tool descriptions for Opus/Sonnet.
        # Check config and model name to decide. Saves ~2K tokens/call.
        self._compact_mode = self._should_use_compact_mode()
        ...
        # Initialize loom for structured turn recording (nchat Phase 6)
        self._loom = None
        if self._compact_mode:
            try:
                from nchat.loom import get_or_create_loom
                self._loom = get_or_create_loom(self.session_id)
            except Exception as e:
                logger.debug("Loom init failed (non-fatal): %s", e)
        ...
        def _run_review():
            ...
            try:
                with open(_os.devnull, "w") as _devnull, \
                    ...
                    # Use WORKER crystal for review instead of PRIMARY.
                    # This is the single biggest cost win in the fork:
                    # review was running on Opus, now runs on Sonnet.
                    from nchat.crystals import resolve_crystal, CrystalTier
                    _crystal = resolve_crystal(
                        CrystalTier.WORKER,
                        parent_agent=self,
                    )
                    review_agent = AIAgent(
                        model=_crystal.model,
                        base_url=_crystal.base_url or None,
                        api_key=_crystal.api_key or None,
                        provider=_crystal.provider or self.provider,
                        api_mode=_crystal.api_mode,
                        max_iterations=8,
                        quiet_mode=True,
    ...
    def _should_use_compact_mode(self) -> bool:
        """Determine if compact system prompt + tool descriptions should be used.

        Compact mode is used for strong models (Opus, Sonnet) that don't need
        verbose behavioral guidance. Saves ~3K+ tokens per API call.
        """
        # Check explicit config first
        try:
            from hermes_cli.config import load_config
            config = load_config()
            mode = config.get("system_prompt_mode", "auto")
            if mode == "compact":
                return True
            if mode == "verbose":
                return False
        ...
        env_mode = os.environ.get("NCHAT_PROMPT_MODE", "").lower()
        if env_mode == "compact":
            return True
        if env_mode == "verbose":
            return False
        ...
        compact_indicators = [
            "opus", "sonnet", "gpt-5", "gpt-4o", "gpt-4-turbo",
            "claude-3.5", "claude-4", "claude-opus", "claude-sonnet",
            "gemini-2.5-pro", "gemini-3-pro",
        ]
        return any(ind in model_lower for ind in compact_indicators)

    # ── nchat loop migration: bridge + cast methods ───────────────────

    def _make_loop_api_call(self, api_messages: list) -> tuple:
        """API call with retry, fallback, and interrupt handling.

        Extracted from run_conversation()'s inner retry loop for use by
        the nchat run_cast() loop via the AgentBridge.

        Returns:
            (response, usage_dict) on success.

        Raises:
            InterruptedError: User interrupted during API call or backoff.
            nchat.loop.ContextOverflowError: Context too large, needs compression.
            nchat.loop.MaxRetriesError: All retries exhausted.
        """
        from nchat.loop import ContextOverflowError, MaxRetriesError

        retry_count = 0
        max_retries = 3

        while retry_count <= max_retries:
            # Interrupt check before each attempt
            ...
            try:
                ...
                if response_invalid:
                    ...
                    if retry_count > max_retries:
                        ...
                        raise MaxRetriesError(
                            "Invalid API response shape after retries"
                        )
                    wait_time = min(5 * (2 ** (retry_count - 1)), 120)
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    usage_dict["prompt_tokens"] = canonical.prompt_tokens
                    usage_dict["completion_tokens"] = canonical.output_tokens
                ...
                # Check for context-length / payload-too-large errors
                ...
                if is_context_error:
                    raise ContextOverflowError(str(api_error))
                ...
                _RETRYABLE = {413, 429, 529}
                if (isinstance(status_code, int)
                        and 400 <= status_code < 500
                        and status_code not in _RETRYABLE):
                    if self._try_activate_fallback():
                        retry_count = 0
                        continue
                    raise MaxRetriesError(
                ...
                logger.warning(
                    "Retrying API call in %ss (attempt %s/%s): %s",
                    wait_time, retry_count, max_retries, api_error,
                )
                self._interruptible_sleep(wait_time)

        # Should not reach here, but safety fallback
        raise MaxRetriesError("API call loop exited unexpectedly")

    def _interruptible_sleep(self, seconds: float) -> None:
        """Sleep in small increments, raising InterruptedError if interrupted."""
        end = time.time() + seconds
        while time.time() < end:
            if self._interrupt_requested:
                raise InterruptedError(
                    "Agent interrupted during retry backoff"
                )
            time.sleep(0.2)

    def _normalize_api_response(self, response) -> tuple:
        """Normalize an API response across all three API modes.

        Returns: (assistant_message_obj, finish_reason_str)
        """
        if self.api_mode == "codex_responses":
    ...
    def _build_loop_api_messages(
        self, system_prompt: str, messages: list,
    ) -> list:
        """Prepare messages for the API, handling reasoning, caching, sanitization.

        Wraps the message preparation logic from run_conversation() for
        ...
        for msg in messages:
            ...
            # Strip Codex fields for strict providers
            if hasattr(self, '_base_url_lower') and "api.mistral.ai" in self._base_url_lower:
        ...
        # Sanitize orphaned tool calls/results
    ...
    def _build_agent_bridge(self, effective_task_id: str) -> "AgentBridge":
        ...
        The bridge provides typed access to AIAgent infrastructure
        without coupling run_cast() to the full AIAgent class.
        """
        from nchat.loop import AgentBridge

        return AgentBridge(
            query_api=self._make_loop_api_call,
            normalize_response=self._normalize_api_response,
            build_assistant_message=self._build_assistant_message,
            execute_tools=self._execute_tool_calls,
            build_api_messages=self._build_loop_api_messages,
            compress_context=lambda msgs, sys_msg, approx, task_id: (
                self._compress_context(
                    msgs, sys_msg, approx_tokens=approx, task_id=task_id,
                )
            ),
            save_session_log=self._save_session_log,
            is_interrupted=lambda: self._interrupt_requested,
            has_stream_consumers=self._has_stream_consumers,
    ...
    def _run_via_cast(
        ...
        """Run the compact-mode loop via nchat/loop.py's run_cast().
        ...
        # Build the circle (gates + wards)
        circle = load_circle_from_config()

        # Build the bridge
        bridge = self._build_agent_bridge(effective_task_id)

        # Resolve crystal info for loom recording
        from nchat.crystals import Crystal, CrystalTier
        crystal = Crystal(
            tier=CrystalTier.PRIMARY,
        ...
        # Post-cast review (memory + skills)
        if final_response and not interrupted:
            try:
                tool_call_count = result.tool_calls_total
                if circle.should_review(result.turns, tool_call_count):
        ...
        if interrupted and self._interrupt_message:
            result_dict["interrupt_message"] = self._interrupt_message
    ...
    def _build_system_prompt_compact(
        ...
        if self._memory_store:
            if self._memory_enabled:
                memory_snapshot = self._memory_store.format_for_system_prompt("memory")
            if self._user_profile_enabled:
                user_profile = self._memory_store.format_for_system_prompt("user")
        ...
        # Honcho block (if active)
        honcho_block = None
        if self._honcho and self._honcho_session_key:
            hcfg = self._honcho_config
            mode = hcfg.memory_mode if hcfg else "hybrid"
            honcho_block = f"# Honcho memory\nActive. Session: {self._honcho_session_key}. Mode: {mode}."

        return build_system_prompt_compact(
            gate_names=sorted(self.valid_tool_names),
            gate_descriptions=gate_descriptions,
            memory_snapshot=memory_snapshot,
            user_profile=user_profile,
            skills_index=skills_index,
            session_id=self.session_id if self.pass_session_id else None,
            model=self.model,
            platform=self.platform,
            system_message=system_message,
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        """
        # COMPACT MODE: use nchat.call for clean, minimal system prompts.
        # The soul is the soul. The circle presents its own capabilities.
        if self._compact_mode:
            return self._build_system_prompt_compact(system_message)
    ...
    def _get_budget_warning(self, api_call_count: int) -> Optional[str]:
        ...
        In compact mode (ward system): single message at 80% mark.
        In verbose mode: two-tier system (70% caution, 90% warning).
        """
    ...
    def run_conversation(
        ...
        #
        # In compact mode, nudges are disabled — review happens post-cast
        # via the PostCastReview ward (see nchat/wards.py).
        ...
        # ── Compact-mode delegation to nchat/loop.py ──
        # When compact mode is active (Opus/Sonnet), the new clean loop handles
        # the turn cycle with per-turn loom recording, ward-based constraints,
        # and bridge-mediated access to AIAgent infrastructure.
        if self._compact_mode:
            return self._run_via_cast(
        ...
        # In compact mode, skill nudges are disabled (post-cast review handles this).
